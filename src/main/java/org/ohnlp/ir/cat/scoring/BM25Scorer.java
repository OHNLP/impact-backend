...
public class BM25Scorer extends Scorer {
    ...
    public PCollection<KV<KV<String, String>, CandidateScore>> score(Pipeline p, Map<String, EntityCriterion> query, ClinicalEntityType queryType, ClinicalResourceDataSource dataSource) {
        ...
        PCollection<KV<String, Long>> doclen = allRecordsByPatientUID.apply("BM25 Step 2.1: Count records by Patient ID", Count.perKey());
        ...
        Schema idfSchema = Schema.of(Schema.Field.of("criterion_uid", Schema.FieldType.STRING), Schema.Field.of("idf", Schema.FieldType.DOUBLE));

        PCollection<Row> idf = getIdf(idfSchema, queryMatches, collSizeView);

        // Calculate term frequency per criterion
        Schema tfSchema = Schema.of(Schema.Field.of("criterion_uid", Schema.FieldType.STRING), Schema.Field.of("patient_uid", Schema.FieldType.STRING), Schema.Field.of("tf", Schema.FieldType.INT64));
    ...
    private PCollection<KV<KV<String, String>, Set<String>>> generateEvidenceSets(PCollection<KV<String, KV<String, DomainResource>>> queryMatches) {
        return queryMatches.apply("Query: generate evidence ID list", new PTransform<PCollection<KV<String, KV<String, DomainResource>>>, PCollection<KV<KV<String, String>, Set<String>>>>() {
            @Override
            public PCollection<KV<KV<String, String>, Set<String>>> expand(PCollection<KV<String, KV<String, DomainResource>>> input) {
                return input.apply("Format matched evidence for grouping and extract IDs",
                        ...
                        ParDo.of(new DoFn<KV<String, KV<String, DomainResource>>, KV<KV<String, String>, String>>() {
                            @ProcessElement
                            public void process(@Element KV<String, KV<String, DomainResource>> in, OutputReceiver<KV<KV<String, String>, String>> out) {
                                out.output(KV.of(KV.of(in.getValue().getKey(), in.getKey()), in.getValue().getValue().getId()));
                            }
                        })).apply("Group IDs by criteria and patient", GroupByKey.create()).apply("Create evidence ID sets",  // ((criterion_uid, patient_uid), evidence_ids)
                        ParDo.of(new DoFn<KV<KV<String, String>, Iterable<String>>, KV<KV<String, String>, Set<String>>>() {
                            @ProcessElement
                            public void process(@Element KV<KV<String, String>, Iterable<String>> in, OutputReceiver<KV<KV<String, String>, Set<String>>> out) {
                                out.output(KV.of(KV.of(in.getKey().getValue(), in.getKey().getKey()), StreamSupport.stream(in.getValue().spliterator(), false).collect(Collectors.toSet())));
                            }
    ...
    private PCollection<Long> getNumPatientsInEHR(PCollection<KV<String, DomainResource>> allRecordsByPatientUID) {
        return allRecordsByPatientUID.apply("BM25 Step 1: Calculate Collection Size", new PTransform<PCollection<KV<String, DomainResource>>, PCollection<Long>>() {
            @Override
            public PCollection<Long> expand(PCollection<KV<String, DomainResource>> input) {
                return input.apply("BM25 Step 1.1: Extract Patient UIDs", ParDo.of(new DoFn<KV<String, DomainResource>, String>() {
                    @ProcessElement
                    public void process(@Element KV<String, DomainResource> e, OutputReceiver<String> out) {
                        out.output(e.getKey());
                    }
                })).apply("BM25 Step 1.2: Dedup Patient UIDs", Distinct.create()).apply("BM25 Step 1.3: Count Dedup'ed Patient UIDs", Count.globally());
            }
        });
    }

    private PCollection<Double> getAverageNumRecordsPerPatient(PCollection<KV<String, Long>> allRecordsByPatientUID) {
        return allRecordsByPatientUID.apply("BM25 Step 2: Calculate Average Collection Length", new PTransform<PCollection<KV<String, Long>>, PCollection<Double>>() {
            @Override
            public PCollection<Double> expand(PCollection<KV<String, Long>> input) {
                return input.apply("BM25 Step 2.2: Extract counts", ParDo.of(new DoFn<KV<String, Long>, Long>() {
                    @ProcessElement
                    public void process(@Element KV<String, Long> e, OutputReceiver<Long> out) {
                        out.output(e.getValue());
                    }
                })).apply("BM25 Step 3.3: Calculate Mean", Mean.globally());
            }
        });
    }
    ...
    private PCollection<Row> getIdf(Schema idfSchema, PCollection<KV<String, KV<String, DomainResource>>> queryMatches, PCollectionView<Long> collSizeView) {
        return queryMatches.apply("BM25 Step 3.1 (IDF): Reformat into (criterionUID, patientUID) pairs", ParDo.of(new DoFn<KV<String, KV<String, DomainResource>>, KV<String, String>>() {
            @ProcessElement
            public void process(@Element KV<String, KV<String, DomainResource>> record, OutputReceiver<KV<String, String>> out) {
                out.output(KV.of(record.getKey(), record.getValue().getKey()));
            }
        })).apply("BM25 Step 3.2 (IDF): Get distinct (criterionUID, patientUID) pairs", Distinct.withRepresentativeValueFn((KV<String, String> kv) -> kv.getKey() + "|" + kv.getValue()).withRepresentativeType(TypeDescriptor.of(String.class))).setCoder(KvCoder.of(StringUtf8Coder.of(), SerializableCoder.of(String.class))).apply("BM25 Step 3.3 (IDF):Get number of patients per criterion", Count.perKey()).setCoder(KvCoder.of(StringUtf8Coder.of(), SerializableCoder.of(Long.class))).apply("BM25 Step 3.4 (IDF): Calculate values", ParDo.of(new DoFn<KV<String, Long>, KV<String, Double>>() {
            @ProcessElement
        ...
        }).withSideInputs(collSizeView)).apply(ParDo.of(new DoFn<KV<String, Double>, Row>() {
            @ProcessElement
    ...
    private PCollection<Row> getTF(Schema tfSchema, PCollection<KV<String, KV<String, DomainResource>>> queryMatches) {
        return queryMatches.apply("BM25 Step 4: Get Term Frequency per Patient (Number of Records Matching Criteria per Patient", new PTransform<PCollection<KV<String, KV<String, DomainResource>>>, PCollection<KV<KV<String, String>, Long>>>() {
            @Override
            public PCollection<KV<KV<String, String>, Long>> expand(PCollection<KV<String, KV<String, DomainResource>>> input) {
                return input.apply(ParDo.of(new DoFn<KV<String, KV<String, DomainResource>>, KV<KV<String, String>, Integer>>() {
                    // Map to <<CriteriaUID, PatientUID>, 1> so we can just count by key
                    @ProcessElement
                    public void process(@Element KV<String, KV<String, DomainResource>> in, OutputReceiver<KV<KV<String, String>, Integer>> out) {
                        out.output(KV.of(KV.of(in.getKey(), in.getValue().getKey()), 1));
                    }
                })).apply(Count.perKey());
            }
        }).apply(ParDo.of(new DoFn<KV<KV<String, String>, Long>, KV<String, KV<String, Long>>>() {
            // Map back to correct format
            @ProcessElement
            public void process(@Element KV<KV<String, String>, Long> in, OutputReceiver<KV<String, KV<String, Long>>> out) {
    ...
    private PCollection<KV<KV<String, String>, CandidateScore>> getScores(PCollection<KV<String, Long>> doclen, PCollection<Row> idf, PCollection<Row> tf, PCollectionView<Double> avgDocLen) {
        Schema docLenSchema = Schema.of(Schema.Field.of("patient_uid", Schema.FieldType.STRING), Schema.Field.of("doc_len", Schema.FieldType.INT64));
        PCollection<Row> doclenColl = doclen.apply(ParDo.of(new DoFn<KV<String, Long>, Row>() {
            @ProcessElement
        ...
        return tf.apply("BM25 Step 5.1: Join TF, IDF, and docLen for calculation",
                ...
                new PTransform<PCollection<Row>, PCollection<Row>>() {
                    ...
                    public PCollection<Row> expand(PCollection<Row> inputTF) {
                        ...
                        return Join.innerBroadcastJoin(doclenColl).on(Join.FieldsEqual.left("lhs.patient_uid").right("patient_uid")).expand(tfIDFJoined);
                    }
                }).apply("BM25 Step 5.2: Calculate BM25 scores from joined values", ParDo.of(new DoFn<Row, KV<KV<String, String>, CandidateScore>>() {
            ...
            public void process(ProcessContext c, @Element Row in, OutputReceiver<KV<KV<String, String>, CandidateScore>> out) {
                double bm25 = bm25(in.getRow("lhs").getRow("rhs").getDouble("idf"), in.getRow("lhs").getRow("lhs").getInt64("tf").doubleValue(), in.getRow("rhs").getInt64("doc_len").doubleValue(), c.sideInput(avgDocLen));
                out.output(KV.of(KV.of(in.getRow("lhs").getRow("lhs").getString("criterion_uid"), in.getRow("lhs").getRow("lhs").getString("patient_uid")), new CandidateScore(in.getRow("lhs").getRow("lhs").getString("patient_uid"), bm25, new HashSet<>() //  Empty evidence/to be added in next step
                )));
            }
    ...
    private PCollection<KV<KV<String, String>, CandidateScore>> joinScoresWithEvidence(PCollection<KV<KV<String, String>, CandidateScore>> scores, PCollection<KV<KV<String, String>, Set<String>>> evidence) {
        ...
        return KeyedPCollectionTuple.of(evidenceTag, evidence).and(scoreTag, scores).apply(CoGroupByKey.create()).apply("Merge evidence IDs into scores", ParDo.of(new DoFn<KV<KV<String, String>, CoGbkResult>, KV<KV<String, String>, CandidateScore>>() {
            @ProcessElement
            public void process(@Element KV<KV<String, String>, CoGbkResult> e, OutputReceiver<KV<KV<String, String>, CandidateScore>> out) {
